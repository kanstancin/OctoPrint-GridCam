...
def get_det_res(buffer, show=False):
    ...
    orig_shape = im_avg1.shape
    im_avg1 = cv.resize(im_avg1, (480, 360), interpolation=cv.INTER_AREA)
    im_avg2 = cv.resize(im_avg2, (480, 360), interpolation=cv.INTER_AREA)
    ...
    maskAOI = getAOIMask(img_shape=orig_shape, poly_pts=pts)
    maskAOI = cv.resize(maskAOI, (480, 360), interpolation=cv.INTER_AREA)
    print("s", maskAOI.shape)
    ...
    # im2 = cv.resize(im2, (orig_shape[1], orig_shape[0]), interpolation=cv.INTER_AREA)
